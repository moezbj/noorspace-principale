...
export default function Teacher() {
  const [section, setSection] = React.useState<string>(() => {
    ...
    const match = hash.startsWith('teacher/') ? hash.split('/')[1] : 'dashboard';
    ...
    <div className="space-y-6">
      <section className="rounded-[14px] overflow-hidden" style={{ backgroundImage: 'linear-gradient(66deg, rgba(233,230,206,1) 3%, rgba(237,127,88,1) 88%)' }}>
        <div className="p-6 flex items-center justify-between">
          <div>
            <div className="text-xl font-semibold tracking-tight text-gray-800">Teacher</div>
      ...
      <section className="bg-[#FBF7F4] border border-ns-border rounded-[18px] p-6">
        ...
        <p className="mt-2 text-sm text-ns-muted">Routes for the Teacher role: {Object.keys(SECTIONS).join(', ')}.</p>
